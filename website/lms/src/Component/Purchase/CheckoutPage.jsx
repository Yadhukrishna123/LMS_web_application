...
const CheckoutPage = () => {
  ...
  const studeDetail = async () => {
    ...
    try {
      setLoading(true)
      let studentDetail = await axios.get(`${import.meta.env.VITE_API_URL}/view_students`)
  ...
  const handlePay = async () => {
    ...
    if (courseDetail.isFree === false) {
      const { data: keyData } = await axios.get(`${import.meta.env.VITE_API_URL}/get_key`)
      const { key } = keyData
      console.log("key:", key);

      const orderRes = await axios.post(`${import.meta.env.VITE_API_URL}/create_payment`, {
      ...
      const options = {
        ...
        handler: async function (response) {
          ...
          try {
            const res = await axios.post(`${import.meta.env.VITE_API_URL}/save_db`, paymentInfo)
            console.log(res)
