...
const Header = () => {
    ...
    const [isDropdownOpen, setIsDropdownOpen] = useState(false); // lookup dropdown
    const [openProfile, setOpenProfile] = useState(false); // desktop profile dropdown
            ...
            <div className="max-w-7xl mx-auto px-4 py-6 flex items-center justify-between">
                ...
                <div className='hidden lg:flex items-center space-x-8'>
                    {user ? (
                        ...
                        <>
                            <Link to="/login" className="px-6 py-2.5 text-indigo-600 font-semibold rounded-full hover:bg-indigo-50 transition-colors">
                                Sign In
                            </Link>
                            <Link to="/sign_up" className="px-6 py-2.5 bg-indigo-600 text-white font-semibold rounded-full hover:bg-indigo-700 transition-colors shadow-md">
                                Sign Up
                            </Link>
                        </>
                    )}
            ...
            {isOpen && (
                <div className="lg:hidden bg-white border-t border-gray-100">
                    <nav className="flex flex-col p-4 space-y-1">
                        ...
                        {user ? (
                            <>
                                <Link to="/user_page" className="mt-2 w-full bg-indigo-50 text-indigo-600 px-4 py-3 rounded-lg hover:bg-indigo-100 font-semibold text-center">Profile</Link>
                                <button onClick={() => setLogout(true)} className="w-full bg-red-50 text-red-600 px-4 py-3 rounded-lg hover:bg-red-100 font-semibold">Logout</button>
                            </>
                        ) : (
                            <>
